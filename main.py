...
def experiment(algorithm:str, method:str, n_c:List[int],n_cl:int,propcl:List[float],propg:float,seed:int,timming = False,n_iter=5):
    ...
    if type(timming) == str or not timming:
        if algorithm == 'HILL CLIMBING':
            ...
            if method == 'ORDERED':
                initial_state = gen_initial_state_ordered(Parameters(n_c,n_cl,propcl,propg,seed))
                n = simulated_annealing(CentralDistributionProblem(initial_state, use_one_action=True), schedule=exp_schedule(k= 1, lam= 0.005, limit=2400))
                #print(f"La representació del estat inicial és aquesta \n {initial_state}")
        ...
        if algorithm == 'HILL CLIMBING':
            ...
            if method == 'ORDERED':
                ...
                return timeit.timeit(lambda: simulated_annealing(CentralDistributionProblem
                    (gen_initial_state_ordered(Parameters(n_c, n_cl, propcl, propg, seed)),use_one_action=True), schedule=exp_schedule(k= 1, lam= 0.005, limit=2400)), number=n_iter), False
            if method == 'ONLY GRANTED':
                ...
                return timeit.timeit(lambda: simulated_annealing(CentralDistributionProblem
                                                          (gen_initial_state_only_granted(
                                                              Parameters(n_c, n_cl, propcl, propg, seed)),use_one_action=True), schedule=exp_schedule(k= 1, lam= 0.005, limit=2400)), number=n_iter), False
